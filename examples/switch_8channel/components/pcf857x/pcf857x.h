 ...
 */
esp_err_t pcf857x_init_desc(i2c_dev_t *dev, i2c_port_t port, uint8_t addr, gpio_num_t sda_gpio, gpio_num_t scl_gpio);
 ...
 * @return `ESP_OK` on success
 */
esp_err_t pcf857x_free_desc(i2c_dev_t *dev);
...
inline esp_err_t pcf8575_port_read(const i2c_dev_t *dev, uint16_t *val)
{
    return i2c_dev_read(dev, NULL, 0, val, 2);
}
...
inline esp_err_t pcf8575_port_write(const i2c_dev_t *dev, uint16_t value)
{
    return i2c_dev_write(dev, NULL, 0, &value, 2);
}
